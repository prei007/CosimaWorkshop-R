...
Materials for a workshop on semantic literature reviews: Producing a knowledge graph (KG) on the basis of an analysis of the literature.

Downloads: 

* PDF Handout [on Dropbox](https://www.dropbox.com/s/dpur88faudcoxfn/Cosima_March_2023_Handout.pdf?dl=0)
* This R Project [on Github](https://github.com/prei007/CosimaWorkshop-R) https://github.com/prei007/CosimaWorkshop-R
...
-   While 'the paper' might be subject to copyright laws, the data can be freely shared and re-used.
-   While a paper publication is not update-able, the data that are underlying the publication can be updated at any time.
-   Since KGs are a knowledge technology, we get features such as definition of meaning and automated reasoning.
    -   This can, for example, be used to automate the thematic coding and to perform quality checks.
...
![One source for multiple uses](images/one-source-many-forms.jpg){width="1500"}
...
### The literature review example

This analytical literature review draws together research from 2011 to 2021 on the use of computer simulations in environmental education. It aims to answer two questions: (1) What simulation models are used for environmental and climate science education? And (2) What are the pedagogical strategies for including simulations and computational modeling? Fifty-one studies were identified as relevant to the questions and of sufficient quality.
...
In our case we start from a RDF file (in so-called [Turtle](https://en.wikipedia.org/wiki/Turtle_(syntax)) format) that encodes references in Dublin Core (DC). This is not sufficient for bioliographic nuances, but sufficient for our purposes. The nuances can stay in a reference manager such as Zotero or Endnote.

*Note:* Zotero uses XML as the serialization format for RDF, not Turtle, which is much more readable for humans. To transform data to different serialization formats, use a converter such as [EasyRDF](https://www.easyrdf.org/converter).
...
-   Use a comma to separate multiple values (objects) for the same predicate: `dc:creator "Hmelo-Silver, Cindy E.", "Liu, Lei" ;`
-   End a series of statements about the same subject with a full stop: `.`

> Exercise: Add a line with predicate `dc:language` and object `EN` to the above description in [this google doc](https://docs.google.com/document/d/1UU2EpJrzANqw_rDBr72KGjkfgXCmIOXusroX1zhO8S8/edit?usp=sharing) (Exercise 1)

#### Namespaces explained

The `dc:` prefix indicates that the predicate, such as `dc:creator`, stems from the [Dublin Core](https://www.dublincore.org/specifications/dublin-core/dcmi-terms/) standard; this is a very established standard for meta-data on media assets. Using standard vocabularies such as DC is essential for exchanging information over the Internet.

The bibliographic data for the references are stored in the file `data/compumod_references_dc.ttl`. The content should be pretty straightforward, other than those pesky namespace prefixes.\
...
Now we can query the database, for instance for listing all authors:

    SELECT DISTINCT ?author WHERE 
     {?ref dcterms:creator ?author}

> Exercise: Write a query that displays the title of all studies in the database in [this google doc](https://docs.google.com/document/d/1UU2EpJrzANqw_rDBr72KGjkfgXCmIOXusroX1zhO8S8/edit?usp=sharing) (Exercise 2)

Note: You can use this database for yourself, but only in READ mode (i.e, browsing and querying). For WRITE mode access, you'd need to contact [me](mailto:peter.reimann@sydney.edu.au).

There is of course not much point to storing bibliographic information about references in a database that are already stored in (many) other places, such as SCOPUS. Next, we are going to add information to these references that is of relevance for the research synthesis, and not stored anywhere else.

## Step 2: Coding and annotating

Besides the biases in the search and selection phase, a research synthesis can also be subject to bias during the (qualitative) coding of studies. Therefore, it is important to make the coding process reliable and re-producible. Another concern--and the one addressed here--is re-use of coding schemes. Such schemes are knowledge!
...
![The place of SKOS](images/SKOS-high-level.jpg){width="700"}
...
> Exercise: Add a knowledge outcome "SystemsKnowledge" to the Thesaurus in [this google doc](https://docs.google.com/document/d/1UU2EpJrzANqw_rDBr72KGjkfgXCmIOXusroX1zhO8S8/edit?usp=sharing) (Exercise 3)

SKOS has additional language elements for describing concepts and their relations, such as for labels, definitions, and languages:
...
![](images/Dickes2019-learning-outcome.jpg) In our example literature review, we apply additional thematic codes, all organised with SKOS, such as:
...
Once we have the references with the thematic codes available and stored in a graph database, we can do useful analyses on them. If the database is accessible freely then that information---the service, actually---will be available to anybody with a web browser.

The full database is accessible at:

`http://learn-web.com/#/catalogs/coolfutures/repositories/compumod`
...
The query language [SPARQL](https://en.wikipedia.org/wiki/SPARQL) is using the Turtle RDF notation to retrieve information from one or more knowledge graphs.
    ...
    SELECT ?title ?year ?dateAdded
        { ?work dc:title ?title . 
        ?work dc:date ?year .
        ?work :technology :Game . 
         OPTIONAL {?work :dateAdded ?dateAdded .} 
        }
    ORDER BY ?work

> Question: Can you discern what this query is looking for?
>
> Exercise: Feel free to search for different technologies, such as `:GIS` or `:ComputerModel`. The relevant thesaurus is `thesauri/EdTechThesaurus.skos.ttl`.
>
> Question: What does the colon (`:`) in front of the thesauri terms stand for?
...
A fun way to play with SPARQL is [DBpedia](https://www.dbpedia.org/), the RDF version of Wikipedia. One can access one of its [SPARQL endpoints](https://www.dbpedia.org/resources/sparql/) and search for information such as:
...
> Pause a moment to think about how you would do this on Wikipedia.
...
> Exercise: What gets returned when the query contains `skos:narrower+` instead of `skos:narrower*`?
...
> Question: How does coding based on INSERT queries contribute to making the coding steps reproducible?
...
This requires to use a programming language, such as JavaScript or Python. My examples are written in R. Details will need to be left to a 'advanced' workshop where the topic of program access will be covered in more detail.
...
### Interactive applications (advanced topic) 
...
The coding that we performed so far can be achieved with data that take the form of attributes (modeled as predicates) and their values (modeled as objects): `<subject> :technology :Game` for instance. In this section, we are going to look into how we can add information that does not fit into the attribute/value format. We begin with free text annotations. 
...
### Modelling a design research project

Relevant files: design-coding.ttl, ontologies/letro.owl.ttl

This is work in progress. I am developing an ontology--the Learning Technology Research Ontology (LETRO)--for describing knowledge created in learning technology research projects, grounded in design science. Its purpose is broader than academic publishing: a tool for managing knowledge on the level of a single project and across projects, i.e., on the level of a research discipline. The motivation is that (learning) design knowledge is difficult to capture in print publications.In Design-based Research publications, for instance, the description of the learning (technology) design is usually rudimentary and lacks any standards. 
...
The definition for a particular design project can be found in the file `design-coding.ttl`.
...
The concepts and relations used for describing a design research project are defined in a (at this stage only rudimentary) formal ontology that can be found in the file `ontologies/letro.owl.ttl`. This ontololgy is best viewed (and authored) with specialised applications such as [Protége](https://protege.stanford.edu/).
